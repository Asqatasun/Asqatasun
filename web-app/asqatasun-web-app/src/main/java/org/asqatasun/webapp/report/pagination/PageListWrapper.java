...
public class PageListWrapper extends TableDecorator {
    ...
    public String getRawMark() {
        ...
        if (lObject instanceof PageResult && !((PageResult) lObject).rawMark.equalsIgnoreCase("-1")) {
            return "<div> " + ((PageResult) lObject).rawMark + PERCENT_CHAR + " </div>";
        }
    ...
    public String getWeightedMark() {
        Object lObject = this.getCurrentRowObject();
        if (lObject instanceof PageResult && !((PageResult) lObject).weightedMark.equalsIgnoreCase("-1")) {
            return "<div>" + ((PageResult) lObject).weightedMark + PERCENT_CHAR + " </div>";
        }
        return "";
    }
    ...
    public String getUrlWithPageResultLinkAndExternalLink() {
        ...
        strb.append(buildExternalLinkUrl(pageResult != null? pageResult.url : null, resourceBundle));
    ...
    public String getUrlLink(
        ...
        int lId = pageResult.id.intValue();
        String url = pageResult.url.replaceAll("&", "&amp;");
        Integer mark = Integer.valueOf(pageResult.rawMark);
        if (mark > 0) {

...
<div id="synthesis-result" class="row">
    ...
    <div id="synthesis-meta-data" class="span8 offset0">
        <div id="project-url">
            <c:choose>
                <c:when test="${statistics.auditScope == 'GROUPOFFILES' 
                                || statistics.auditScope == 'FILE'}">
                    <span class="synthesis-meta-title">
                        <fmt:message key="auditSetUp.file"/> : 
                    </span>${statistics.url}
                </c:when>
                ...
                <c:otherwise>
                    <span class="synthesis-meta-title">
                        Url : 
                    </span><a href="${statistics.url}">${statistics.url}</a>
                </c:otherwise>
            </c:choose>
            <c:if test="${addLinkToSourceCode == 'true'}">
                ...
                <span id="page-source-code-link">    
                    <a href="<c:url value="/home/contract/source-code-page.html?wr=${param.wr}"/>" 
                       title="${sourceCodeLinkTitle}" 
                       target="_blank" 
                       class="result-page-action">
                        Code&nbsp;HTML
                    </a>
                </span>
            </c:if>
        ...
        <div id="project-creation-date">
            <span class="synthesis-meta-title">
                <fmt:message key="contract.createdOn"/> : 
            </span>
        ...
        <c:if test="${hasPageCounter == 'true'}">
            ...
            <div class="audit-nb-of-pages">
                ...
                <c:choose>
                    <c:when test="${hasPagesListLink == 'true'}">
                        <c:choose>
                            <c:when test="${statistics.auditScope == 'SCENARIO'}">
                <a href="<c:url value="/home/contract/page-list.html?audit=${param.audit}&amp;status=f2xx&amp;sortDirection=2&amp;sortCriterion=rank"/>" >
                ...
                <a href="<c:url value="/home/contract/page-list.html?audit=${param.audit}"/>">
                    ${pageCounterLinkTitle}
                </a>
                <span>
                    (${totalPageCounterTitle})
                </span>
                            </c:when>
                        </c:choose>
                    </c:when>
            ...
            <c:if test="${hasSiteScopeTest == 'true'}">
                <div id="synthesis-action-list">
                    <a href="<c:url value="/home/contract/site-result.html?wr=${wr}"/>" 
                       class="result-page-action">
                        <spring:message code="synthesisSite.siteResults"/>
                    </a>
                </div>
            </c:if>                                        
        </c:if>
        <c:if test="${fn:length(statistics.parametersMap) != 0}">
            ...
            <div id="audit-level">
                ...
                <span class="synthesis-meta-value">
                    <spring:message code="${levelCode}"/>
                </span>
            </div>
            <div>
                <span id="master-audit-parameters">
            ...
            <div id="audit-parameters">
                <ul>
                    <c:forEach var="entry" 
                               items="${statistics.parametersMap}">
...
<c:if test="${hasResultDispatchTitle == 'true'}">
    <div class="row">
        <div class="span16 offset0">
            <h2 id="result-dispatch">
                <spring:message code="synthesisSite.resultDispatch"/>
            </h2>
        </div><!-- class="span16 offset0" -->
    </div> <!-- class="row" -->
</c:if>
    ...
    <div id="graphical-synthesis-result" class="row">
        <c:if test="${hasPieChartInGraphicalResult == 'true'}">
            <div class="span5">
                <c:set var="counter" 
                       scope="request" 
                       value="${statistics.resultCounter}"/>
                <c:import url="template/pie-chart.jsp"/>
            </div><!---class="span5"-->
        </c:if>
        <c:set var="counterByThemeMap" 
               scope="request" 
               value="${statistics.counterByThemeMap}"/>
        <c:set var="width" 
               scope="request" 
               value="${themeRepartitionWidth}"/>
        <c:set var="height" 
               scope="request" 
               value="250"/>
        <c:set var="xLabel" 

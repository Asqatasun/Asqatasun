                ...
                <table id="result-by-theme" class="zebra-striped">
                    ...
                    <thead>
                        ...
                        <c:forEach var="entry" items="${counterByThemeMap}">
                            <th id="category${entry.key.rank}" title="<fmt:message key="${entry.key.code}"/>"><fmt:message key="${entry.key.code}"/></th>
                        </c:forEach>
                        </tr>    
                    </thead>
                    <tbody>
                        <tr>
                            <th id="serie1" title="<fmt:message key="failed"/>"><fmt:message key="failed"/></th>
                            <c:forEach var="entry" items="${counterByThemeMap}">    
                            <td headers="serie1 category${entry.key.rank}">${entry.value.failedCount}</td>
                            </c:forEach>
                        </tr>
                        <tr>
                            <th id="serie2" title="<fmt:message key="passed"/>"><fmt:message key="passed"/></th>
                            <c:forEach var="entry" items="${counterByThemeMap}">    

		...
		<table id="top5-failed-theme" summary="<fmt:message key="graph.mostInvalidedThemesSummaryAndCaption"/>">
                    ...
                    <tr>
                        ...
                        <c:forEach var="counterByTheme" items="${counterByThemeMap}" varStatus="categoriesIndex">
                            ...
                            <th id="top5Theme${categoriesIndex.index}" title="<fmt:message key="${counterByTheme.key.code}"/>"><fmt:message key="${counterByTheme.key.code}"/></th>
                            </c:if>
                        </c:forEach>
                    </tr>
                    <tr>
                        <th id="top5ResultFailed"><fmt:message key="failed"/></th>
                        <c:forEach var="counterByTheme" items="${counterByThemeMap}" varStatus="categoriesIndex">
                            <c:if test="${categoriesIndex.index < 5}">
                            <td headers="top5ResultFailed top5Theme${categoriesIndex.index}">${counterByTheme.value.failedCount}</td>
                            </c:if>
                        </c:forEach>
                    </tr>
                </table>

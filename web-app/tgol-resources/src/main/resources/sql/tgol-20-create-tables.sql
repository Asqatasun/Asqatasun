...
CREATE SCHEMA IF NOT EXISTS `$tgDatabase` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci ;
USE `$tgDatabase` ;
...
CREATE TABLE IF NOT EXISTS `TGSI_ACT_WEB_RESOURCE` (
  ...
  INDEX `INDEX_TGSI_ACT_WEB_RESOURCE_ACT_Id_Act` (`ACT_Id_Act` ASC) ,
  INDEX `INDEX_TGSI_ACT_WEB_RESOURCE_WEB_RESOURCE_Id_Web_Resource` (`WEB_RESOURCE_Id_Web_Resource` ASC) ,
  UNIQUE INDEX `INDEX_UNIQUE_PAIR` (`ACT_Id_Act`, `WEB_RESOURCE_Id_Web_Resource`),
  CONSTRAINT `FK_TGSI_ACT_WEB_RESOURCE_TGSI_ACT`

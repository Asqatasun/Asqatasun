...
public class Rgaa30Rule080401Test extends Rgaa30RuleImplementationTestCase {
    ...
    protected void setProcess() {
        ...
        ee = pIter.next();
        assertTrue(ee.getValue().contains("Rgaa30 Test.8.4.1 Failed 05 Rgaa30 Test.8.4.1 Failed 05 Failed : the default language is defined on the html tag but its"));
        ...
        assertEquals(EvidenceStore.DETECTED_LANGUAGE_EE, ee.getEvidence().getCode());
        ee = pIter.next();
        assertTrue(ee.getValue().contains("Rgaa30 Test.8.4.1 NMI 01 (testcase warning one) Rgaa30 Test.8.4.1 NMI 01 (testcase warning one)"));
        assertEquals(EvidenceStore.EXTRACTED_TEXT_EE, ee.getEvidence().getCode());

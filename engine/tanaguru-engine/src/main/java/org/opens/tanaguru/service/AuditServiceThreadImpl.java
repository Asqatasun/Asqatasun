...
public class AuditServiceThreadImpl implements AuditServiceThread {
    ...
    @Override
    public void analyse() {
        audit = getAuditDataService().getAuditWithWebResource(audit.getId());
        ...
        if (parentWebResource instanceof Page) {
            getAnalyserService().analyse(parentWebResource);
            getWebResourceDataService().saveOrUpdate(parentWebResource);
            ...
            Long nbOfContent =
                    getWebResourceDataService().getNumberOfChildWebResource(parentWebResource);
            Long i = Long.valueOf(0);
            ...
            while (i.compareTo(nbOfContent) < 0) {
                ...
                webResourceList = getWebResourceDataService().getWebResourceFromItsParent(
                        parentWebResource,
                        i.intValue(),
                ...
                for (WebResource webResource : webResourceList) {
                    ...
                    getAnalyserService().analyse(webResource);
                    if (LOGGER.isDebugEnabled()) {
                        endPersistDate = Calendar.getInstance().getTime();
        ...
        audit = getAuditDataService().saveOrUpdate(audit);
    }

...
scp "ci/${UPDATE_SCRIPT}" "$USER"@"$HOST":~/"$REMOTE_DIR"
...
ssh "$USER"@"$HOST" bash -c "'cd $REMOTE_DIR && APP_VERSION=$APP_VERSION CI_PROJECT_NAME=$CI_PROJECT_NAME WEBAPP_ARTIFACT_FILE=$WEBAPP_ARTIFACT_FILE REST_SERVER_ARTIFACT_FILE=$REST_SERVER_ARTIFACT_FILE ./${UPDATE_SCRIPT}'"
